...
struct PendingProposal: Identifiable {
    ...
    let summary: String
    let taskId: String  // The task this proposal belongs to
    let projectId: String  // The project this proposal belongs to
    ...
    private var migrator: DatabaseMigrator {
        ...
        migrator.registerMigration("v1") { db in
            ...
            try db.create(table: "artifact") { t in
                ...
                t.column("items", .text) // JSON array for collection items
            }
            try db.create(index: "artifact_taskId", on: "artifact", columns: ["taskId"])
            
            // App state table (for persisting active project, etc.)
        ...
        migrator.registerMigration("v2") { db in
            // Add persona column to messages
        ...
        migrator.registerMigration("v4") { db in
            // Add sourceUrl column to projects
            try db.alter(table: "project") { t in
        ...
        migrator.registerMigration("v5") { db in
            // Add localPath column to projects
            try db.alter(table: "project") { t in
        ...
        migrator.registerMigration("v6") { db in
            // Add language and cachedHighlight for code artifacts
        ...
        migrator.registerMigration("v7") { db in
            // Engineer criteria - independent from tasks
...
struct DBEngineerCriterion: Codable, FetchableRecord, PersistableRecord, Identifiable {
    ...
    var isValidated: Bool { isCompleted }  // Alias for compatibility
}
...
struct DBMessage: Codable, FetchableRecord, PersistableRecord, Identifiable {
    ...
    var text: String
    var timestamp: Date
    var persona: String  // "manager" or "engineer" - which persona this was sent to/from
...
struct DBArtifact: Codable, FetchableRecord, PersistableRecord, Identifiable {
    ...
    var id: String
    var taskId: String
    var type: String          // "markdown", "code", "image", "csv"
    var label: String
    var file: String          // Content (markdown text, code, base64 image, csv content)
    var path: String?         // File path (for large files or directories)
    var description: String?  // Deliverable marker or notes
    var order: Int
    var maxRows: Int?         // CSV: max preview rows (default 10, max 100)
    var language: String?     // Code: syntax highlighting language
    var cachedHighlight: String? // Unused, kept for DB compat
    var caption: String?      // Description/caption for all artifact types
    ...
    func loadAllTaskArtifacts() {
        do {
            let allArtifacts = try db.read { db in
                try DBArtifact.filter(Column("description") != "deliverable" || Column("description") == nil).fetchAll(db)
            }
    ...
    func loadAllTaskCriteria() {
        do {
            let allCriteria = try db.read { db in
                try DBCriterion.order(Column("order")).fetchAll(db)
            }
...
// Old JSON structures for migration
